...
{
    public class OrderCachePolicy : ICachePolicy
    {
        public double ExpirationTimeInDays { get; set; }

        public OrderCachePolicy(double expirationTimeInDays = 0)
        {
        ...
        public CacheItemPolicy GetMonitorCachePolicy()
        {
            ...
            {
                using (var command = new SqlCommand("SELECT [OrderID] FROM [NorthwindEF].[Northwind].[Orders]", conn))
                {
